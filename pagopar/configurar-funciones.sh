...
case "$RETORNO" in
  # PagoPar exige el marcador CON paréntesis: ($hash). Si no lo encuentra tal
  # cual, rechaza la URL al guardarla. Se aceptan también las otras formas por
  # si algún día cambian el formato.
  *'($hash)'* | *'$hash'* | *'{hash}'*)
